...
def _build_context(docs: List[Document], max_chars: int = 4000) -> str:
    """Builds the formatted context block for the LLM, including key metadata."""
    chunks = []
    total = 0
...
def _compute_similarity(
    ...
    """
    Compute similarity scores between a query vector and document vectors.

    Supported metrics:
      - cosine:      angle-based, scale-invariant
      - dot_product:  raw inner product
      - euclidean:   1 / (1 + L2_distance), so higher = more similar
    """
    if metric == "dot_product":
        return doc_vecs @ q_vec
    elif metric == "euclidean":
        dists = np.linalg.norm(doc_vecs - q_vec, axis=1)
        return 1.0 / (1.0 + dists)
    else:  # cosine (default)
        q_norm = np.linalg.norm(q_vec)
        doc_norms = np.linalg.norm(doc_vecs, axis=1)
        denom = np.maximum(q_norm * doc_norms, 1e-8)
        return (doc_vecs @ q_vec) / denom
...
def _similarity_rank_and_filter(
    ...
    """
    Rank docs by the chosen similarity metric and filter below min_sim.
    """
    log_lines: List[str] = []
    ...
    sims = _compute_similarity(q_vec, doc_vecs, metric)
    ...
    if not indices:
        log_lines.append(
            f"Similarity filtering ({metric}): {num_raw} raw docs → 0 kept "
    ...
    log_lines.append(
        f"Similarity filtering + reranking (metric={metric}):\n"
...
def _retrieve_documents_from_db(
    ...
    if getattr(config, "use_rerank", False): # Se la checkbox è SPUNTATA
        ...
        reranked_docs, sim_log = _similarity_rank_and_filter(
            ...
            metric=getattr(config, "rerank_metric", "cosine"),
        )
...
def _decide_need_retrieval(
    ...
    """
    Intelligent Router (ALIGNED WITH MULTI-AGENT):
...
def _single_agent_answer_question_core(
    ...
    if context:
        ...
        # --- CASE B: NO CONTEXT AVAILABLE ---
        if need_retrieval:
            # Legal question BUT without documents (or DB Selection = NONE)
            base_instructions = (
