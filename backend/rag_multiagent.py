...
def _decide_need_retrieval(
    ...
    """
    Intelligent Router:
    - YES: Specific questions about Divorce/Inheritance in IT/EE/SI that require documents.
    - NO: Chitchat, Off-topic, or Generic legal definitions (Theory).
    """
    system_prompt = (
        ...
        "User: 'Diffexrence between void and voidable contract?' -> NO\n"
    ...
    # Conservative fallback: when in doubt, search.
...
def _multiagent_answer_question_core(
    ...
    # --- STEP 1: INTELLIGENT ROUTER (Legal vs General) ---
    ...
    # --- STEP 3: NON-LEGAL QUESTIONS HANDLING (Chitchat) ---
    if not need_retrieval:
        # Direct response without using sub-agents or DB
        system_prompt = (
    ...
    # =========================================================================
    # FROM HERE ON: CLASSIC MULTI-AGENT LOGIC (Only for valid Legal questions)
    # =========================================================================
    ...
    # Metadata Filters Extraction (for sub-agents)
    ...
    # --- FALLBACK IF NO AGENT RESPONDS ---
    if not per_agent_answers:
        # If we're here, the Router said "YES" (Legal), but the DB Supervisor said "NONE".
        # We execute a fallback to Single Agent BUT ensuring it sees ALL DBs.
        
        # Note: single_agent_answer_question uses config.vector_store_dirs if present.
        # We need to ensure the config passed to fallback has all paths.
