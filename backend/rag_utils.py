...
# =====================================================================
# DOMAIN CONTROL AND METADATA FILTERING FUNCTIONS
# =====================================================================
...
def _describe_databases(
    ...
    """
    SHORT description for DBs extracting countries and laws from FAISS metadata.
    USES RANDOM SAMPLING TO AVOID BIAS ON SORTED DATASETS.
    """
    ...
    for db_name, path in db_map.items():
        try:
            ...
            if hasattr(vs, "docstore") and hasattr(vs, "index_to_docstore_id"):
                ...
                # --- CRUCIAL MODIFICATION: RANDOM SAMPLE ---
                # If we only take the first 50 and the dataset is sorted by country,
                # we lose the other countries.
        ...
        # Build the specific description for the Supervisor
        parts = []
        
        # 1. Specify covered countries
        if countries:
            parts.append(f"countries: {', '.join(sorted(countries))}")
        else:
            parts.append("countries: ALL/UNKNOWN")  # Fallback if no metadata found

        # 2. Specify the legal matter
        if laws:
            parts.append(f"law: {', '.join(sorted(laws))}")
            
        # 3. Specify the content nature based on DB name
...
def _decide_which_dbs(
    ...
    """
    LLM Supervisor decides which DBs to use.
    """
