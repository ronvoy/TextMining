...
def build_four_vector_stores(config: RAGConfig):
    ...
    for law_type in law_types:
        ...
        # Your document_loader.py is already efficient in loading Codes and filtering Cases by Law Type
        docs = load_documents_by_law_type(config, law_type)
        
        if not docs:
            print(f"  No documents found for {law_type}, skipping...")
            continue
        ...
        if codes:
            ...
            print(f"  Skipping {law_type} Codes store: 0 documents found.")
        ...
        if cases:
            ...
            print(f"  Skipping {law_type} Cases store: 0 documents found.")
        ...
        if not country_path.exists():
            ...
            for doc_type in ["divorce", "inheritance", "cases"]:
                ...
                if json_files:
                    ...
                    print(f"       {doc_type}: no files found")
    
    return True

# Note: 'shutil' is already imported at the top with other imports 

def clean_all_vector_stores(config: RAGConfig):
    """
    Removes ALL Vector Store directories inside the base directory.
    """
    base_dir = Path(config.vector_store_base_dir)
    print("\n" + "="*80)
    print(f"CLEANUP: Removing ALL directories in {base_dir.name}/")
    
    deleted_count = 0
    
    # Find all subdirectories inside the base directory
    # This includes 'divorce', 'inheritance', 'divorce_codes', 'divorce_cases', etc.
    for item in base_dir.iterdir():
        if item.is_dir():
            ...
            try:
                ...
                print(f"   Error while deleting {item.name}: {e}")
                
    if deleted_count > 0:
        print(f" Cleanup completed. Removed {deleted_count} Vector Stores in total.")
    else:
        print(" No Vector Store directories found to remove.")
    print("="*80)
...
def main():
    ...
    # -----------------------------------------------------------------
    # CALL FOR COMPLETE CLEANUP
    # -----------------------------------------------------------------
    clean_all_vector_stores(config)  # <--- THIS FUNCTION DELETES EVERYTHING
    # -----------------------------------------------------------------
    
    print("\n" + "="*80)
    input("\nPress ENTER to start building 4 vector stores from scratch (this will take a few minutes)...")
    
    # CALL TO REBUILD THE NEW 4 DBs
